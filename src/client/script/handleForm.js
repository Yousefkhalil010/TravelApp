...
const getCityLoc = async () => {
  if (cityInp.value) {
    const { data } = await axios.post("http://localhost:8081/getCity", form, {
      headers: {
...
const getWeather = async (lng, lat, remainingDays) => {
  const { data } = await axios.post("http://localhost:8081/getWeather", {
    lng,
    lat,
    remainingDays,
  });
...
const getCityPic = async (city_name) => {
  const { data } = await axios.post("http://localhost:8081/getCityPic", {
    city_name,
  });
